...
class Board:
            ...
            if self.move == 'left':
                ...
                for i in range(self.width):
                    ...
                    for j in range(self.height):
                        ...
                        pygame.draw.rect(screen, pygame.Color('white'), [(i * self.cell_size + self.left,
                                        j * self.cell_size + self.top), (self.cell_size, self.cell_size)], 2)

                        if self.boarda1[j][i] - self.y > 0:
                            if self.board[j][i] != 0:
                                pygame.draw.rect(screen, self.colors[self.board[j][i]],
                                                 [(i * self.cell_size + self.left + 2,
                                                   j * self.cell_size + self.top + 2 - self.y),
                                                  (self.cell_size - 3, self.cell_size - 3)], 0)
                                text = font.render(str(self.board[j][i]), 1, (pygame.Color("white")))
                                screen.blit(text,
                                            (i * self.cell_size + self.left + 2 + (100 - text.get_width()) // 2,
    ...
    def endgame(self):
        text = font.render(str(board.score), 1, (pygame.Color("white")))
        screen.blit(text, (
            250 + (100 - text.get_width()) // 2,
            150 + (100 - text.get_height()) // 2))
        text = font.render("YOUR SCORE", 1, (pygame.Color("white")))
        screen.blit(text, (
            250 + (100 - text.get_width()) // 2,
            100 + (100 - text.get_height()) // 2))
        text = font.render("WASTED", 1, (pygame.Color("red")))
        screen.blit(text, (
            250 + (100 - text.get_width()) // 2,
            10 + (100 - text.get_height()) // 2))
        text = font.render('PRESS "F" TO START NEW GAME', 1, (pygame.Color("white")))
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and f:
            if event.key == pygame.K_LEFT and not start:
                board.checkendleft()
                f = False
            ...
            elif event.key == pygame.K_DOWN and not start:
                board.checkendown()
                f = False

            elif event.key == pygame.K_f and t:
                board = Board(4, 4)
                f = True
                t = False
                start = False
    ...
    #score
    if start:
        text = font.render('PRESS "F" TO START', 1, (pygame.Color("white")))
        ...
        if not t:
            text = font.render(str(board.score), 1, (pygame.Color("white")))
            screen.blit(text, (
                450 + (100 - text.get_width()) // 2,
                50 + (100 - text.get_height()) // 2))
            text = font.render("SCORE", 1, (pygame.Color("white")))
            screen.blit(text, (
                450 + (100 - text.get_width()) // 2,
                10 + (100 - text.get_height()) // 2))
        if t:
            ...
            if f:
                ...
                board.checkendright()
                board.checkenup()
                if board.boarda1 == [[0 for i in range(board.width)] for j in range(board.height)]:
                    t = True
                    board.boarda1 = [[0 for i in range(board.width)] for j in range(board.height)]
                else:
                    board.boarda1 = [[0 for i in range(board.width)] for j in range(board.height)]
